...
for sound in arrayOfNoiseMaker {
    sound.makeNoise()
}
...
*/
struct TwoDimArray: Sequence {
    let array: [[Int]]
    
    func makeIterator() -> TwoDimArrayIterator {
        return TwoDimArrayIterator(self)
    }
...
struct TwoDimArrayIterator: IteratorProtocol {
    var times = 0
    var array: TwoDimArray
    
    init(_ array:TwoDimArray){
        self.array = array
    }
...
let numbers = TwoDimArray(array: [[2,5,9], [0,4,2], [6,8,3]])
